...
VOID Inject(PVOID DllImage) {
    ...
    // get process id
	//==========================================
    ULONG ProcessID = GetProcID("Bandizip.exe");
	DWORD threadID;
	while (!ProcessID) {
		threadID = GetWindowThreadProcessId(FindWindowA("Bandizip (Standard)", NULL), &ProcessID);//skCrypt("Call of Duty?HQ") "gfx_test"
		Sleep(10);
	}
	...
	if (ProcessID) {
		...
		//获取目标进程中最后一个线程栈的位置，作为DLL分配基址的参考。
		...
		//==========================================
		if (AllocationBase) {
			//强制在目标进程中分配一个虚拟地址描述符（VAD）用于DLL注入。
			//VAD分配成功后，AllocationBase成为DLL镜像的加载地址。
		...
		//==========================================
		//根据目标进程内存中实际的AllocationBase地址修复DLL的重定位表。///////////////////////////////////////********************
		//解决DLL的地址与加载地址不一致的问题。
		...
		//				Fix IAT
		//==========================================
		//修复DLL的导入地址表（IAT），使其在目标进程内指向正确的函数地址。
		...
		if (FlipExecuteBit((HANDLE)ProcessID, AllocationBase, 0)) {
			//劫持目标进程中的线程（通过线程栈或任意代码执行），调用DLL的DllMain函数。
		...
		//==========================================
		//删除DLL中已加载到目标进程但不再需要的部分（如.reloc节）。
...
void CloseDiscord() {
	...
	if (Process32First(snapshot, &entry) == TRUE)
	{
		...
		{
			if (wcsicmp(entry.szExeFile, L"discord.exe") == 0)
			{
