...
NTSTATUS Memory::InitializeFuncs(){
	...
	PVOID DriverBase = Util::GetDriverBase(sDriverName, &BaseInfo);



	MiAllocateVad = (PMMVAD_SHORT(*)(UINT_PTR, UINT_PTR, LOGICAL))Util::FindPatternImage((PCHAR)BaseInfo.BaseAddress, MI_ALLOCATE_VAD_PATTERN, MI_ALLOCATE_VAD_MASK);
	...
	MiInsertVad = (VOID(*)(PMMVAD_SHORT, PEPROCESS))Util::FindPatternImage((PCHAR)BaseInfo.BaseAddress, MI_INSERT_VAD_PATTERN, MI_INSERT_VAD_MASK);
	...
	MiInsertVadCharges = (NTSTATUS(*)(PMMVAD_SHORT, PEPROCESS))Util::FindPatternImage((PCHAR)BaseInfo.BaseAddress, MI_INSERT_VAD_CHANGES_PATTERN, MI_INSERT_VAD_CHANGES_MASK);
...
//AllocateVAD 函数的作用是为指定进程分配一个新的虚拟地址描述符（VAD），并配置该地址的保护属性。
NTSTATUS Memory::VAD::AllocateVAD(HANDLE ProcessID, PVOID Address, SIZE_T Size) { // size is size of our dll
	...
	// 设置VAD的保护属性（例如，6 可能表示可执行属性）
	Flags->Protection = (6);
	// 将 NoChange 标志设为0（表示不允许更改保护属性）
	Flags->NoChange = 0;
