...
__int64 NTAPI Comms(PVOID Request, PINT64 Status)
{
	...
	在此函数中，首先会检查 Request 数据是否合法。如果不合法，调用原始函数 EnumerateDebuggingDevicesOriginal。
    如果合法，会将 gEnumDebugDeviceFunc 指向原始函数，并进入 Comm::Loop() 通信循环。也就是说，在这时，原始函数的执行会被推迟，直到 Comm::Loop 完成。
	...
	*/
	...
	// 把它指向 EnumerateDebuggingDevicesOriginal 函数
...
{
	auto Base = (PCHAR)Memory::GetKernelModule("ntoskrnl.exe");


	PBYTE FunctionAddress = static_cast<PBYTE>(Util::FindPatternImage(Base, NT_ADD_FONT_PATTERN,
		NT_ADD_FONT_MASK));
	if (!FunctionAddress)
	{
...
VOID Comm::Loop()
{
	...
	for (;;)
	{
		// 如果进程的活动线程标志为 1，表示该线程已经结束或不再活动，释放进程对象并返回
