...
//通过修改目标进程内存中页表项的 ExecuteDisable 位来修改进程的执行权限。
BOOL Util::SpoofPTE(PVOID Address, HANDLE ProcessID) {
	...
	if (NT_SUCCESS(Status)) {
		...
		// 遍历内存区域的每个 4KB 页（系统通常以 4KB 为单位映射虚拟内存）
		for (ULONGLONG i = reinterpret_cast<ULONGLONG>(Mbi.BaseAddress);
			...
			// 如果找到了有效的 PTE 且该页处于“Present”（即内存页已加载到物理内存中）
			if (PTE && PTE->Present) {

				// 修改 PTE 的 ExecuteDisable 位，禁用该页的执行权限
			   // 设置 ExecuteDisable 为 0 允许该页执行
...
UCHAR Util::RandomNum() {
	...
	PVOID Base = Util::GetDriverBase("ntoskrnl.exe", NULL);

	auto cMmGetSystemRoutineAddress = reinterpret_cast<decltype(&MmGetSystemRoutineAddress)>(Util::GetExportedFunction((ULONGLONG)Base, "MmGetSystemRoutineAddress"));
...
//获取指定进程最后一个线程的栈基地址和线程 ID
PVOID Util::Process::GetLastThreadStack(HANDLE ProcessID, PVOID pOut, PVOID pThreadID) {
	// `ProcessID`：目标进程的进程ID
   // `pOut`：输出参数，接收线程的栈基地址
   // `pThreadID`：输出参数，接收线程ID

	PVOID ThreadID = 0;// 用于存储找到的线程ID
	NTSTATUS status = STATUS_SUCCESS;
	PEPROCESS Process;
	PsLookupProcessByProcessId(ProcessID, &Process);

	PVOID TebBases[100]{ 0 };// 存储线程Teb基地址的数组

	// 获取目标进程中所有线程的TEB基地址
	GetThreadTEBs(ProcessID, TebBases, &ThreadID);

	// 存储每个线程栈的边界（Stack Limit）
	PVOID StackLimits[100]{ 0 };

	// 遍历所有线程，读取其栈限制
	for (int i = 0; i < 100; i++) {
		if (!TebBases[i])// 如果Teb地址为空，跳过
			continue;

		// 读取线程的NT_TIB结构（线程信息块），以获取栈限制
		NT_TIB TIB{ 0 };
...
NTSTATUS Util::Process::RemoveVAD(HANDLE ProcessID, PVOID Address) {
	...
	PMMVAD_SHORT pVadShort = NULL; // 用于存储找到的VAD条目
	...
	if (NT_SUCCESS(Status)) {
		...
		return STATUS_UNSUCCESSFUL;// 如果找不到VAD，则返回失败状态
	}
...
//该函数可用于通过内核代码修改目标进程的页表，使得进程可以访问某些通常受保护的内核内存区域
BOOL Util::ExposeKernelMemory(HANDLE ProcessID, uintptr_t KernelAddress, SIZE_T Size) {
	...
	for (uintptr_t Address = KernelAddress; Address <= KernelAddress + Size; Address += 0x1000) {
		...
		// 设置页目录项（PDE）、页目录指针项（PDPTE）和页表项（PML4E）的 Supervisor 位 为1表示这些项对内核是可访问的。
		...
		if (!PageInfo.PDE || (PageInfo.PTE && !PageInfo.PTE->Present)) {
			...
			// 如果页表项有效，则设置其 Supervisor 位为 1，表示可访问
